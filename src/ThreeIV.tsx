...
export default function ThreeII() {
            ...
            for (let x = -20; x < 20; x++) {
              for (let y = -20; y < 20; y++) {
                for (let z = 0; z < 5; z++) {
                  ...
                  sprite.position.set(x * 4, y * 4, z * 40);
                  refScene.current.add(sprite);
                }
              }
            }
          }
  ...
  function onMouseMove(event: MouseEvent) {
    if (mouseDown && refCamera.current && refScene.current) {
      ...
      // 根据鼠标移动的距离调整相机的旋转
      // refCamera.current.position.x -= deltaX * 0.1;
      // refCamera.current.position.y += deltaY * 0.1;
      refScene.current.rotation.x += deltaY * 0.01;
